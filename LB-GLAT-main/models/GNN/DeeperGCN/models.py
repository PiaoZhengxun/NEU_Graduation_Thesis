...
class SingleGraphSAGEModel(nn.Module):
    ...
    def __init__(self, gnn_layer_num, linear_layer_num, n_features, n_classes,
                 gnns_hidden: Tensor, linears_hidden: Tensor, gnn_do_bn, linear_do_bn,
                 gnn_dropout, linear_dropout, device, bias=True):
        super(SingleGraphSAGEModel, self).__init__()
        self.device = device
        # GNN using GraphSAGE
        self.Sage = SAGEsBlock(layer_num=gnn_layer_num, n_features=n_features,
                               gnns_hidden=gnns_hidden, do_bn=gnn_do_bn, dropout=gnn_dropout, bias=bias)
    ...
    def forward(self, x, edge_index, mask=None):
        # edge_index 자꾸 바뀌는거 방지 [2. ....] 으로 바꿈
        # if edge_index.shape[0] != 2:
        #     edge_index = edge_index.t()
        # Graph SAGE Forward Pass

        # print(f"Input x shape: {x.shape}")
        # print(f"Input mask shape (if any): {mask.shape if mask is not None else None}")

        graph_embeddings = self.Sage(x, edge_index)

        if mask is not None:
            graph_embeddings = [emb[mask] for emb in graph_embeddings]

            # Apply Fully Connected Layers
        output = self.fc(graph_embeddings[-1])  # Use the final layer output
        return output
...
class SAGEsBlock(nn.Module):
    """Graph SAGE block similar to GENsBlock"""
