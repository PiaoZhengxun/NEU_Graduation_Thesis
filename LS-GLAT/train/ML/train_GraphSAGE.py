...
def train_graphsage(model, train_x, train_y, edge_index, epochs, learning_rate, device):
...
if __name__ == "__main__":
    ...
    if isinstance(edge_index, list):
        edge_index = np.concatenate(edge_index, axis=1) if isinstance(edge_index[0], np.ndarray) else np.array(edge_index)
    ...
    model = GraphSAGEModel(config["input_dim"], config["hidden_dim"], config["output_dim"])
    train_graphsage(
        model,
